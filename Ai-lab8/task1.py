import numpy as np
import tensorflow.compat.v1 as tf
import matplotlib.pyplot as plt
import warnings

tf.disable_v2_behavior()
tf.reset_default_graph()
warnings.filterwarnings("ignore")

n_samples, batch_size, num_steps = 1000, 100, 20000

X_data = np.random.uniform(0, 1, (n_samples, 1))
y_data = 2 * X_data + 1 + np.random.normal(0, 0.2, (n_samples, 1))

X = tf.placeholder(tf.float32, shape=(batch_size, 1))
y = tf.placeholder(tf.float32, shape=(batch_size, 1))

with tf.variable_scope('linear-regression'):
    k = tf.Variable(tf.random_normal((1, 1)), name='slope')
    b = tf.Variable(tf.zeros((1,)), name='bias')

y_pred = tf.matmul(X, k) + b
loss = tf.reduce_sum((y - y_pred) ** 2)

optimizer = tf.train.GradientDescentOptimizer(learning_rate=0.005).minimize(loss)

display_step = 2000

with tf.Session() as sess:
    sess.run(tf.global_variables_initializer())
    
    for i in range(num_steps):
        indices = np.random.choice(n_samples, batch_size)
        X_batch, y_batch = X_data[indices], y_data[indices]
        
        _, loss_val, k_val, b_val = sess.run([optimizer, loss, k, b], feed_dict={X: X_batch, y: y_batch})
        
        if (i + 1) % display_step == 0:
            print('Епоха %d: %.8f, k=%.4f, b=%.4f' % (i + 1, loss_val, k_val[0][0], b_val[0]))
            
    final_k = k_val[0][0]
    final_b = b_val[0]

plt.figure(figsize=(10, 6))
plt.scatter(X_data, y_data, alpha=0.3, label='Вхідні дані (з шумом)')

x_line = np.linspace(0, 1, 100)
y_line = final_k * x_line + final_b

plt.plot(x_line, y_line, color='red', linewidth=3, label='Навчена регресія')

plt.title('Результат навчання лінійної регресії в TensorFlow')
plt.legend()
plt.grid(True)
plt.show()
